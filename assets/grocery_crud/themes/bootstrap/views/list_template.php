                ...
                <div class="table-container">
                    <?php echo form_open("", 'method="post" autocomplete="off" id="gcrud-search-form"'); ?>
                        ...
                        <div class="scroll-if-required">
        			        <table class="table table-bordered grocery-crud-table table-hover tablesaw table-striped table-bordered table-hover tablesaw-swipe tablesaw-sortable">
        					<thead>
        						<tr>
        							<th colspan="2" <?php if ($buttons_counter === 0) {?>class="hidden"<?php }?> width="100px">
                                        <?php echo $this->l('list_actions'); ?>
                                    </th>
                                    <?php foreach($columns as $column){?>
                                        <th class="column-with-ordering" data-order-by="<?php echo $column->field_name; ?>" 
                                        <?php echo ( isset($this->style_string[$column->field_name]) ) ? 'style="'.$this->style_string[$column->field_name].'"' : '';?> ><?php echo $column->display_as; ?>
    <?php #var_dump($column); ?>
